...
class PathPlanner(threading.Thread):
	...
	Then PathPlanner generated shortest path is sent to tracker via queue_tracker
	"""
	def __init__(self):
                #""the construct"""
		threading.Thread.__init__(self)
		#generate instance variables of queues:
		self.myQueue=MyQueue.MyQueue()
		self.queue_pathPlanner = self.myQueue.queue_pathPlanner
	...
	def run(self):	
		...
		while 1:
			sender, command, receivedData= self.queue_pathPlanner.get()
			#mylogger.info(command)
			if command == "quit":
				mylogger.info("Going down.")
				break
			if sender == "worldModelClient":	
				if command == "WMD":			# newWorldModelData
					...
					#~ mylogger.info("received obstacleList: " +str(obstacleList))
					#~ mylogger.info("received vehicleList: " +str(vehicleList))
					#~ mylogger.info("received chessList: " +str(chessList))
					#~ mylogger.info("wmTime: " +str(wmTime))
					...
					self.testMethod()
                                        ...
                                        #print MDcoordinate
					mylogger.info("Shortest Parth:" +str(MDcoordinate))
					
					#send path to Tracker
					# ... has to be done
			if sender == "pe":
				#do some stuff with new position estimation data
				#~ mylogger.info("Received Data from Positionestimator: " + str(receiveData))
				pass
			if sender== "ui":
				if command == "motionCommand":
					mylogger.info("got motion Command from user Interface")
	...
	#	mylogger.info("Hi")
                   ...
                   for k in range(len(self.X[i])):           #k for points of an object
                        for j in range(len(self.X1)-4*(i+1)):        #j for points of other objects
                          # a, b are two points to which line to be drawn or not, while, c,d are nearest two points of a
                            ...
                            #print len(self.X1)
                            #pdb.set_trace()
                            if ((self.X[a/4][a%4]  in self.Xovlap) and (self.Y[a/4][a%4]  in self.Yovlap)) or ((self.X[b/4][b%4] in self.Xovlap) and (self.Y[b/4][b%4]  in self.Yovlap)):
                                draw=0 #pass #break
                            else:
        ...
        def MinpathCoordinate(self, SP):
                """returning Min-path in the form of coordinates"""
                #print SP
                if (SP !=[]) and (SP != None):
                    del SP[0]
                    CoordinateSP=[]
